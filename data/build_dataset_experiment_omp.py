# Steps to create the dataset for author written experimental programs in OpenMP:
...
def main():
    ...
    modules = ['bubble_sort', 'counting_sort', 'odd-even', 'quick_sort']
    output_file = open("dataset_large.csv", "a")
    output_file.write(
    ...
    for module in modules:
        ...
        for size in input_size:
            for thread in threads:
                ...
                shell_command = subprocess.Popen('./' + module + ' ' + str(thread) + ' ' + str(size), shell=True,
                ...
                shell_command = subprocess.Popen(
                    'perf stat -o output_large.txt --field-separator=, -e branch-instructions,branch-misses,cache-misses,cache-references,cycles,instructions,cpu-clock,page-faults,L1-dcache-loads,L1-icache-load-misses,LLC-load-misses -- ./' + module + ' ' + str(
                        thread) + ' ' + str(size), shell=True, stdout=subprocess.PIPE)
                shell_command.wait()
                with open("output_large.txt", "r") as f:
                    lines = f.readlines()
                for line in lines[2:]:
                    values = line.split(",")
                    value = values[0]
